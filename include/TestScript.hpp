...
namespace rosa {
    ...
    class TestScript : public NativeScriptEntity {
        public:
            explicit TestScript(Scene* scene, Entity* entity) : NativeScriptEntity(scene, entity) {}

            void onCreate() override {
                spdlog::info("Test script initialised");
                auto& texture = ResourceManager::instance().getTexture("assets/rosa.png");
                getEntity().getComponent<SpriteComponent>().sprite.setTexture(texture);
                ...
                getEntity().getComponent<TransformComponent>().position = position;
            }
